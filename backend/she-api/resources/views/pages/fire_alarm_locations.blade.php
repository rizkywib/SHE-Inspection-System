...
@section('title', 'Fire Alarm Locations')
@section('nav-fire-alarm-locations', 'active')

@section('content')
<div class="p-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Fire Alarm Locations</h1>
        <p class="text-gray-600 mt-1">Manage fire alarm location master data</p>
    </div>
...
async function loadLocations() {
    const res = await fetch(`${API_URL}/fire-alarm-locations`, {
...
document.getElementById('locationForm').addEventListener('submit', async event => {
    ...
    const res = await fetch(id ? `${API_URL}/fire-alarm-locations/${id}` : `${API_URL}/fire-alarm-locations`, {
        method: id ? 'PUT' : 'POST',
...
async function deleteItem(id) {
    ...
    const res = await fetch(`${API_URL}/fire-alarm-locations/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
    });

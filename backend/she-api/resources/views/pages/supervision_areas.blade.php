...
@section('title', 'Area Pengawasan')
@section('nav-supervision-areas', 'active')

@section('content')
<div class="p-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Area Pengawasan</h1>
        <p class="text-gray-600 mt-1">Kelola daftar area pengawasan untuk dropdown Permit Matrix</p>
    </div>
    ...
    <div class="mb-6">
        <button onclick="openForm()" class="btn-primary text-white px-6 py-3 rounded-lg shadow-md">
            <i class="fas fa-plus mr-2"></i>New Area Pengawasan
        </button>
    </div>

    <div id="formCard" class="hidden bg-white rounded-xl shadow-lg p-6 mb-6">
        <h2 id="formTitle" class="text-xl font-bold text-gray-900 mb-4">Create Area Pengawasan</h2>
        <form id="areaForm" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input id="code" required maxlength="50" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Contoh: 1">
                    <p class="text-xs text-gray-500 mt-1">Kode ini yang tampil di dropdown Area Pengawasan Permit Matrix.</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Name <span class="text-red-500">*</span></label>
                    <input id="name" required maxlength="255" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Nama area pengawasan">
                </div>
                <div>
...
async function loadAreas() {
    try {
        const res = await fetch(`${API_URL}/supervision-areas`, {
        ...
        showMessage(error.message || 'Data Area Pengawasan gagal dimuat.', 'error');
    }
}
...
function renderTable() {
    ...
    if (totalItems === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="px-6 py-8 text-center text-gray-500">No area pengawasan found</td></tr>';
    } else {
        tbody.innerHTML = pageItems.map((area, index) => `
            <tr class="hover:bg-gray-50 transition">
                <td class="px-6 py-4 text-sm text-gray-900">${start + index + 1}</td>
                <td class="px-6 py-4 text-sm font-mono text-gray-900 font-medium">${escapeHtml(area.code)}</td>
...
function openForm() {
    ...
    document.getElementById('formTitle').textContent = 'Create Area Pengawasan';
...
function editItem(id) {
    ...
    openForm();
    document.getElementById('formTitle').textContent = 'Edit Area Pengawasan';
...
document.getElementById('areaForm').addEventListener('submit', async e => {
    ...
    const url = id ? `${API_URL}/supervision-areas/${id}` : `${API_URL}/supervision-areas`;
    ...
    try {
        ...
        showMessage(result.message || (id
            ? 'Area Pengawasan updated successfully.'
            : 'Area Pengawasan saved successfully.'));
    } catch (error) {
...
async function deleteItem(id) {
    if (!confirm('Delete this area pengawasan?')) return;
    try {
        const res = await fetch(`${API_URL}/supervision-areas/${id}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
        });
        ...
        await loadAreas();
        showMessage(result.message || 'Area Pengawasan deleted successfully.');
    } catch (error) {

...
async function loadAlarms() {
    ...
    tbody.innerHTML = alarms.map((a, i) => `<tr class="hover:bg-gray-50 transition"><td class="px-6 py-4 text-sm text-gray-900">${i + 1}</td><td class="px-6 py-4 text-sm text-gray-900 font-medium"><button onclick="editItem(${a.id})" class="text-blue-600 hover:text-blue-800 hover:underline font-medium">${escapeHtml((a.location && a.location.name) || (a.area && a.area.name) || '-')}</button></td><td class="px-6 py-4 text-sm text-gray-500">${escapeHtml(formatDateOnly(a.inspection_date))}</td><td class="px-6 py-4 text-sm text-gray-500">${escapeHtml((a.inspector && a.inspector.name) || '-')}</td><td class="px-6 py-4 text-sm"><button onclick="editItem(${a.id})" class="text-blue-600 hover:text-blue-800 mr-3 font-medium"><i class="fas fa-edit mr-1"></i>Edit</button><button onclick="deleteItem(${a.id})" class="text-red-600 hover:text-red-800 font-medium"><i class="fas fa-trash mr-1"></i>Delete</button></td></tr>`).join('');
}
function openForm() {

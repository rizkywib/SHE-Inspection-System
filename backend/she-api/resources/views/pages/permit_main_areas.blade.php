...
@section('nav-permit-main-areas', 'active')

@section('content')
<div class="p-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Main Area</h1>
        <p class="text-gray-600 mt-1">Kelola daftar main area untuk dropdown Permit Matrix</p>
    </div>
    ...
    <div class="mb-6">
        <button onclick="openForm()" class="btn-primary text-white px-6 py-3 rounded-lg shadow-md">
            <i class="fas fa-plus mr-2"></i>New Main Area
        </button>
    </div>

    <div id="formCard" class="hidden bg-white rounded-xl shadow-lg p-6 mb-6">
        <h2 id="formTitle" class="text-xl font-bold text-gray-900 mb-4">Create Main Area</h2>
        <form id="areaForm" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input id="name" required maxlength="255" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Contoh: EOB1">
                    <p class="text-xs text-gray-500 mt-1">Nama ini yang tampil di dropdown Main Area Permit Matrix.</p>
                </div>
                <div>
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            ...
            <input id="searchFilter" type="text" placeholder="Search main areas..." class="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent w-64">
        </div>
    </div>
...
async function loadAreas() {
    try {
        ...
        if (!res.ok) {
            ...
            document.getElementById('areaTable').innerHTML = `<tr><td colspan="4" class="px-6 py-8 text-center text-red-500">${escapeHtml(errorText)}</td></tr>`;
        ...
        document.getElementById('areaTable').innerHTML = `<tr><td colspan="4" class="px-6 py-8 text-center text-red-500">Error: ${error.message}</td></tr>`;
        showMessage(error.message || 'Data Main Area gagal dimuat.', 'error');
    }
}
...
function renderTable() {
    ...
    if (totalItems === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="px-6 py-8 text-center text-gray-500">No main area found</td></tr>';
    } else {
...
function openForm() {
    ...
    document.getElementById('formTitle').textContent = 'Create Main Area';
...
function editItem(id) {
    ...
    openForm();
    document.getElementById('formTitle').textContent = 'Edit Main Area';
...
document.getElementById('areaForm').addEventListener('submit', async e => {
    ...
    const url = id ? `${API_URL}/permit-main-areas/${id}` : `${API_URL}/permit-main-areas`;
    ...
    try {
        ...
        showMessage(result.message || (id
            ? 'Main Area updated successfully.'
            : 'Main Area saved successfully.'));
    } catch (error) {
...
async function deleteItem(id) {
    if (!confirm('Delete this main area?')) return;
    try {
        const res = await fetch(`${API_URL}/permit-main-areas/${id}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
        });
        ...
        await loadAreas();
        showMessage(result.message || 'Main Area deleted successfully.');
    } catch (error) {

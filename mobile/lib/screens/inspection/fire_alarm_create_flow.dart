...
class FireAlarmSetupScreen extends StatefulWidget {
  ...
  @override
  State<FireAlarmSetupScreen> createState() => _FireAlarmSetupScreenState();
}

class _FireAlarmSetupScreenState extends State<FireAlarmSetupScreen> {
  ...
  Future<void> _loadLocations() async {
    ...
    try {
      ...
      if (connectivity.isOnline) {
        locations =
            await context.read<ApiService>().getFireAlarmLocations();
        unawaited(OfflineStorageService.instance
            .saveCache('fire_alarm_locations', locations));
      } else {
        locations = await OfflineStorageService.instance
                .readCache('fire_alarm_locations') ??
            [];
      }
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _openScanner() async {
    ...
    final didSave = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => FireAlarmQrScannerScreen(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('New Fire Alarm Inspection')),
      body: SafeArea(
        child: _isLoading
...
class FireAlarmQrScannerScreen extends StatefulWidget {
  const FireAlarmQrScannerScreen({
    super.key,
  ...
  @override
  State<FireAlarmQrScannerScreen> createState() =>
      _FireAlarmQrScannerScreenState();
}

class _FireAlarmQrScannerScreenState extends State<FireAlarmQrScannerScreen> {
    ...
    if (point == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'QR Code tidak terdaftar sebagai titik Fire Alarm.',
            ),
            backgroundColor: Colors.red,
          ),
        );
  ...
  Map<String, dynamic>? _findPoint(String qrValue) {
    ...
    final legacyMatch =
        RegExp(r'^(?:FIRE[_-]?ALARM|ALM):(\d+)$', caseSensitive: false)
            .firstMatch(qrValue);
    pointId ??=
                    ...
                    Positioned(
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            const Text(
                              'Arahkan kamera ke QR Code Fire Alarm',
...
class FireAlarmCreateScreen extends StatefulWidget {
  const FireAlarmCreateScreen({
    super.key,
    required this.inspectionDate,
    required this.locationId,
    required this.locationName,
...
class _FireAlarmCreateScreenState extends State<FireAlarmCreateScreen> {
  final ImagePicker _picker = ImagePicker();
  late final TextEditingController _alarmNumberController;
  ...
  @override
  void dispose() {
    _alarmNumberController.dispose();
  ...
  Future<void> _save() async {
    ...
    final fields = <String, String>{
      ...
      'items[0][correction_needed]': _correctionNeeded ? '1' : '0',
    ...
    try {
      ...
      if (!connectivity.isOnline) {
        ...
        await OfflineStorageService.instance.enqueueDraft(
          endpoint: '/fire-alarms',
          method: 'POST',
          fields: fields,
          files: fileMap,
          displayName: 'Fire Alarm $_alarmName',
        );
        if (!mounted) return;
      ...
      final response = await api.createFireAlarmWithPhotos(
        fields,
        files,
      );
      if (!mounted) return;
      ...
      if (response.containsKey('error')) {
        debugPrint('[AlarmCreate] Server error: ${response['error']}');
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Fire Alarm inspection berhasil disimpan.'),
          backgroundColor: Colors.green,
        ),
      );
      Navigator.pop(context, true);
    } catch (error) {
      debugPrint('[AlarmCreate] Exception: $error');
      await _enqueueRetry(fields, files);
      if (!mounted) return;
      setState(() => _isSaving = false);
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Create Fire Alarm')),
      body: SafeArea(
                ...
                child: Column(
                  children: [
                    _ConditionYesNo(
                      ...
                      onChanged: (value) =>
                          setState(() => _conditionGood = value),
                    ),
                    _ConditionYesNo(
                      label: 'Correction Needed',
                      value: _correctionNeeded,
                      onChanged: (value) =>
                          setState(() => _correctionNeeded = value),
                    ),
                    const SizedBox(height: 12),
                    TextField(

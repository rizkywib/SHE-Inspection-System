...
@section('title', 'Type Permit')
@section('nav-permit-types', 'active')

@section('content')
<div class="p-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Type Permit</h1>
        <p class="text-gray-600 mt-1">Kelola daftar type permit untuk dropdown Permit Matrix</p>
    </div>
    ...
    <div class="mb-6">
        <button onclick="openForm()" class="btn-primary text-white px-6 py-3 rounded-lg shadow-md">
            <i class="fas fa-plus mr-2"></i>New Type Permit
        </button>
    </div>

    <div id="formCard" class="hidden bg-white rounded-xl shadow-lg p-6 mb-6">
        <h2 id="formTitle" class="text-xl font-bold text-gray-900 mb-4">Create Type Permit</h2>
        <form id="typeForm" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input id="name" required maxlength="255" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Contoh: HOT WORK">
                    <p class="text-xs text-gray-500 mt-1">Nama ini yang tampil di dropdown Type Permit Permit Matrix.</p>
                </div>
                <div>
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            ...
            <input id="searchFilter" type="text" placeholder="Search types..." class="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent w-64">
        </div>
    </div>
...
async function loadTypes() {
    try {
        const res = await fetch(`${API_URL}/permit-types`, {
        ...
        if (!res.ok) {
            ...
            document.getElementById('typeTable').innerHTML = `<tr><td colspan="4" class="px-6 py-8 text-center text-red-500">${escapeHtml(errorText)}</td></tr>`;
        ...
        document.getElementById('typeTable').innerHTML = `<tr><td colspan="4" class="px-6 py-8 text-center text-red-500">Error: ${error.message}</td></tr>`;
        showMessage(error.message || 'Data Type Permit gagal dimuat.', 'error');
    }
}
...
function renderTable() {
    ...
    if (search) {
        filtered = types.filter(type => {
            return String(type.id).includes(search) ||
    ...
    if (totalItems === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="px-6 py-8 text-center text-gray-500">No type permit found</td></tr>';
    } else {
        tbody.innerHTML = pageItems.map((type, index) => `
            <tr class="hover:bg-gray-50 transition">
                <td class="px-6 py-4 text-sm text-gray-900">${start + index + 1}</td>
                <td class="px-6 py-4 text-sm text-gray-900 font-medium">${escapeHtml(type.name)}</td>
                <td class="px-6 py-4 text-sm">${statusBadge(type.is_active)}</td>
                <td class="px-6 py-4 text-sm">
                    <button onclick="editItem(${type.id})" class="text-blue-600 hover:text-blue-800 mr-3 font-medium">
                        <i class="fas fa-edit mr-1"></i>Edit
                    </button>
                    <button onclick="deleteItem(${type.id})" class="text-red-600 hover:text-red-800 font-medium">
...
function openForm() {
    ...
    document.getElementById('formTitle').textContent = 'Create Type Permit';
...
function editItem(id) {
    ...
    openForm();
    document.getElementById('formTitle').textContent = 'Edit Type Permit';
    document.getElementById('type_id').value = type.id;
    document.getElementById('name').value = type.name || '';
    document.getElementById('is_active').value = Number(type.is_active) === 1 ? '1' : '0';
}
...
document.getElementById('typeForm').addEventListener('submit', async e => {
    ...
    const url = id ? `${API_URL}/permit-types/${id}` : `${API_URL}/permit-types`;
    ...
    try {
        ...
        showMessage(result.message || (id
            ? 'Type Permit updated successfully.'
            : 'Type Permit saved successfully.'));
    } catch (error) {
...
async function deleteItem(id) {
    if (!confirm('Delete this type permit?')) return;
    try {
        const res = await fetch(`${API_URL}/permit-types/${id}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
        });
        ...
        await loadTypes();
        showMessage(result.message || 'Type Permit deleted successfully.');
    } catch (error) {
...
document.getElementById('nextPage').addEventListener('click', function() {
    const totalItems = types.filter(type => {
        const search = searchFilter.toLowerCase().trim();
